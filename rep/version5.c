...
/* iterate */
  for (k=1; k <= nit; k++)
      {
/* send my leftmost grid point to left neighbor */
    MPI_Isend(&h[1], count, MPI_DOUBLE, left, tag1,
    MPI_COMM_WORLD, &request[0]);
/* receive from right neighbor and put into right ghost buffer */
    MPI_Irecv(&h[n+1], count, MPI_DOUBLE, right, tag1,
    MPI_COMM_WORLD, &request[1]);
/* send my rightmost grid point to right neighbor */
    MPI_Isend(&h[n], count, MPI_DOUBLE, right, tag2,
    MPI_COMM_WORLD, &request[2]);
/* receive from left neighbor and put into left ghost buffer*/
    MPI_Irecv(&h[0], count, MPI_DOUBLE, left, tag2,
